...
The usual way to express an IP Address is using dotted decimal notation, such
as 172.16.10.1, and a prefix, such as 24, separated by a slash. 

  172.16.10.1/24

To create a new IPv4 object, you can use the IPv4 class

  ip = IPAddress::IPv4.new "172.16.10.1/24"

or, an easier way, using the IPAddress parse method

  ip = IPAddress.parse "172.16.10.1/24"

which tries parse any kind of common IP (IPv4, IPV6 and 
IPv4 IPv6 Mapped addresses) notation.
...
You can specify an IPv4 address in two ways:

  IPAddress "172.16.10.1/24"
  IPAddress "172.16.10.1/255.255.255.0"
  
In these examples, the netmask is specified using either prefix notation, ex.
/24, or dotted decimal notation, ex. 255.255.255.0.  They are the same and you
have the flexibility to use either.

If you don't provide the subnet mask, IPAddress will assume you are dealing
with hosts and use a 32-bit mask.  Therefore, the default prefix will be /32,
or 255.255.255.255. For example:

  # Declare a host address.
  host = IPAddress::IPv4.new "10.1.1.1"

  puts host.to_string #=> "10.1.1.1/32"
  
The new IPAddress has prefix /32, which is the same 
doing the following:
...
To retrieve the netmask in dotted-decimal notation, use IPv4#netmask:

  ip.netmask   #=> "255.255.255.0"

IPv4#octets is available to access the four decimal octets from the IP address:

  ip.octets    #=> [172,16,10,1]

The shortcut method IPv4#[], provides access to a given octet:

  ip[1]        #=> 16

To print the IPv4 address in a canonical form, use IPv4#to_string:

  ip.to_string #=> "172.16.10.l/24"

=== Changing netmask
    
Use the IPv4#prefix method to assign new netmask using prefix notation after creating an IPv4 object:

  ip.prefix = 25

  ip.to_string #=> "172.16.10.1/25"
    
To set the netmask using dotted-decimal notation, use the IPv4#netmask= method:
...
Several important concepts when dealing with IP addresses are +network+,
+broadcast+, and the addresses included in a given range.

When you write an IPv4 address in prefix notation, such as "172.16.10.1/24",
you are actually specifying two different pieces of information: 

* The IP address itself, "172.16.10.1".
* The subnet mask, which is used to determine the network.

The subnet mask specifies the number of bits to be used in the network address.
In the example above with a prefix of /24, the network number is therefore the
first 24 bits (0-24) of the IP address, and the host number is the remaining 8
bits (25-32).  Thus, the IP address "172.16.10.1/24" belongs to network
"172.16.10.0/24".

This is very important because, for example, the address "172.16.10.1/16" is
very different to the previous one with a /24 prefix.  "172.16.10.1/16" is on
network "172.16.0.0/16", while "172.16.10.1/24" is on network "172.16.10.0",
and consequently they will be handled differently by routers.
...
Method IPv4#network returns a new IPv4 object from the network number,
calculated using the original IP address. We want to emphasize here that the
network address is a perfect legitimate IPv4 address, which just happens to
have all zeroes in the host portion. 
...
The former is a network because the final decimal octet, 4, is 00000100 in
8-bits of binary.  The final octet is bits 25-32 when considering the address'
entire 32-bits of binary.  The netmask has been specified to be 30-bits, thus
the first six bits of the last octet are taken to be part of the network
address.  Thus the IP address "172.16.10.4/30" is the network boundary.  The
final two bits are host address.

==== Broadcast

The broadcast address is the complement of the network address: where the
...
The netmask allows us to determine a network, and therefore the range of IP
addresses included in that network: all the addresses between the network
number and the broadcast. IPAddress has many methods to iterate between those
addresses.
...
If you only want to iterate over host IP addresses omitting the network and
broadcast addresses, use the IPv4#each_host method:
