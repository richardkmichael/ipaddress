...
module IPAddress; 
  ...
  class IPv4
    ...
    #
    def self.summarize(*ip_addresses)
      # If only one network, the summary is the first network.
      return [ip_addresses.first.network] if ip_addresses.size == 1

      networks = ip_addresses.dup.sort.map { |ip_address| ip_address.network }

      i = 0
      while i < networks.size - 1
        sum = networks[i] + networks[i+1]
        networks[i..i+1] = sum.first if sum.size == 1
        i += 1
      end

      networks.flatten!
      if networks.size == ip_addresses.size
        networks
      else
        summarize(*networks)
      end
